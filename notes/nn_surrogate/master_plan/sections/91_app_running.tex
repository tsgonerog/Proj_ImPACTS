...
\begin{callout}
\textbf{\code{code\_tap/} is currently staged for the plain MPI adjoint} ---
\code{SIZE.h\_mpi}, \code{AUTODIFF\_PARAMS.h\_OG}, \code{the\_model\_main.F\_OG},
\code{autodiff\_readparms.F\_OG}, \code{forward\_step\_b.f\_modified\_mpi} ---
which is what \code{build\_tapAdj/} was compiled against, so that build directory
is internally consistent.

Build directories \emph{symlink} their headers back into \code{code\_tap/}, so
building a different variant re-stages \code{code\_tap/} and silently repoints
\code{build\_tapAdj/}'s headers. For this ensemble: do not build another variant
in between, and never run bare \code{make} in an older build directory ---
re-run its build script instead. To check what a build actually compiled against,
diff its generated \code{.f} file, not its symlinked \code{.h}.
\end{callout}
...
\code{input*/} --- anything placed directly there is staged into every run. The
submit script resolves \code{test\_cases="M<n>"} to \code{variants/data\_M<n>}
and stages only that one.

Forward namelist, \code{input/variants/data\_M<n>}, for the leg from 2170 to
2180:
...
Adjoint namelist, \code{input\_tap/variants/data\_M<n>}, identical to the
reference adjoint apart from the diffusivity file:
...
\code{tools/machine\_env.sh}. Per member, edit \code{test\_cases="M<n>"} at the
top of the script --- and the pickup \code{ln -s} further down, which is not
auto-patched --- then:
