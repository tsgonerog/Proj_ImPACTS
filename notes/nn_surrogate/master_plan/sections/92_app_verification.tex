...
\code{useGrdchk = .TRUE.} was set when this was written, so the check ran on
every adjoint job, its settings held in \code{input\_tap/data.grdchk}. (Since
2026-08-28 the committed DINO \code{data.pkg} sets it \code{.FALSE.}; the
analysis below applies whenever the check is switched back on.) It cannot pass
at its current setting, and it never has.
