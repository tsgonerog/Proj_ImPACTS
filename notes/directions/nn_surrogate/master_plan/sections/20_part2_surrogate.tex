...
reference five-year run (job 31039, from the 180-year pickup, \code{visc2x};
the 2026-09-01 seam-clean rerun --- \code{fc} and gradients bit-identical
along the chain 28486\,$\equiv$\,30995\,$\equiv$\,31039) that is $366$
dumps at five-day spacing (the rerun adds a twelfth dump field, the
free-surface adjoint \code{ADJetan}, 367 dumps; not listed below as it is
...
Parametric, not active & \code{ADJbtdrg}, \code{ADJkapgm}, \code{ADJkapredi}
  & in upstream's TAF dump set (\code{addummy\_in\_stepping.F}), not among the
    11 fields the Tapenade hook dumps; need controls declared and, for the last
    two, GM/Redi enabled\\
\midrule
...
\begin{enumerate}
  ...
  \item \textbf{Tapenade \code{-nocheckpoint} tuning.} Done, and the default
        since 2026-09-02: a fresh profile of the c69m adjoint (run 31053)
        replaced the c69f-era 64-routine list with 33 routines --- the two
        share eight names and none of the top twelve --- and the tuned build
        reproduces the checkpoint-everything one bitwise in \code{fc},
        \code{adxx\_*} and \code{ADJ*} at 30 days and at 5 years while
        running $1.47\times$ faster (9.6\,h against 14.1\,h for a five-year
        window); rerunning all eight ensemble adjoints with it on 2026-09-02
        (jobs 31060--31067) gave the same bitwise identity, blow-ups included,
        at $1.45$--$1.65\times$ (9.5--9.75\,h against 14.0--15.7\,h).
        \code{build\_tapAdj.sh} now points at it; the record is
        \code{tools/tapenade\_profiling/README.md}. It does not apply to the
        \code{adjViscBoost} build, where the same list changes the boosted
        adjoint at order one (run 31056 vs 31025).
\end{enumerate}
...
\code{useGrdchk=.FALSE.} and report both halves separately. Five-year
numbers measured since: 14.0--15.7\,h with every call checkpointed (the
eight ensemble adjoints 31039--31046, 2026-09-01) and 9.5--9.75\,h with the
\code{-nocheckpoint} build that is now the default (run 31055 and the
ensemble rerun 31060--31067, 2026-09-02), so the $8.9$\,h in the table was
close for the tuned build and optimistic for the other.
...
1. Configuration work
  ...
  & Repaired gradient check at $(2,127,26)$; \code{useGrdchk} off for ensemble
    ...
    \emph{Gate passed} (Section~\ref{sec:results}): the response is strong,
    $\kv$ is a required input, and the ensemble additionally exposed an
    adjoint stability bound that now shapes Phase~2's design.\\
