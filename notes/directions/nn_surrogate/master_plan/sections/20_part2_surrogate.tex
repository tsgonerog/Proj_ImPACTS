...
\begin{enumerate}
  ...
  \item \textbf{Tapenade \code{-nocheckpoint} tuning.} Done, and the default
        ...
        window). \code{build\_tapAdj.sh} now points at it; the record is
...
\code{useGrdchk=.FALSE.} and report both halves separately. Two five-year
numbers measured since: 14.1\,h with every call checkpointed (run 31039,
2026-09-01) and 9.6\,h with the \code{-nocheckpoint} build that is now the
default (run 31055), so the $8.9$\,h in the table was close for the tuned
build and optimistic for the other.
