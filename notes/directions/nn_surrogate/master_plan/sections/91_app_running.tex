...
adjoint toolchain has since moved to Tapenade-native hooks (since 2026-09-02
under the upstream hook names \code{DUMMY\_IN\_STEPPING}, \code{DUMMY\_FOR\_ETAN},
\code{AUTODIFF\_INADMODE\_SET/UNSET}, their hand-written adjoints filling the
\code{pkg/tapenade/dummy\_tap.F} stubs): stock \code{genmake2}, no post-edited
generated files (the
...
overrides), but since 2026-08-27 the submit scripts patch the namelist copy
staged in the run directory rather than the tracked one, so the ``Submitting
edits the tracked namelist'' callout below describes behaviour that no longer
occurs. See \code{notes/references/tapenade\_hooks/} for the full change
note.
...
\code{code\_tap/variants/adjointViscosity/} as a second \code{-mods} directory.
