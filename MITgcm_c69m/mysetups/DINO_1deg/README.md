...
active. The same activity-through-arguments pattern also drives the two
adjoint-mode switch hooks (`TAP_INADMODE_SET/UNSET`), which is what makes the
`adjViscBoost` parameters actually engage — before these hooks the TAF-named
`ADAUTODIFF_INADMODE_SET` was never called under Tapenade and adjViscBoost
silently ran plain physics. After `make`, the scripts assert that every
generated `_B` call carries exactly the argument count the hand-written
routines declare (dump hook 25, mode-switch hooks 5 each), and fail loudly
otherwise — F77 would silently misalign a mismatch.

There is no `rawTapenade` control build any more: raw Tapenade output *is* the
working configuration. (SOMA still carries both the control and the old patched
`genmake2` it contrasts with.)
...
`AUTODIFF_INADMODE_SET/UNSET`). Under TAF, `.flow` directives
...
1. **Same-name shadowing.** `genmake2 -mods=../code_tap` puts `code_tap/`
   ...
   `addummy_in_stepping.F`, `autodiff_inadmode_set_ad.F`, `stubs_tap_adj.F`)
...
2. **Shadows are differentiated too.** Whether a source goes to Tapenade is
   ...
   shadowed `forward_step.F` is differentiated exactly as the upstream one
   would be. Conversely, a **new** basename (`tap_dummy_in_stepping.F`,
   `tap_inadmode.F`) appears in no list and compiles as a plain forward
...
4. **Tapenade does the rest.** Seeing active data enter and leave each hook,
   ...
   `forward_step_b.f`, at the exact reverse-sweep mirror of each forward call
   site: the dump call where the old hand-patch used to insert it, the mode
   switches at each backward step's start (apply `inAd*`) and end (restore
   forward parameters — so checkpoint re-forwards always run forward physics).
...
6. **A build-time assertion guards the interface.** F77 checks no signatures,
   ...
   generated call's arguments (`check_gen_call`: dump hook 25, mode switches
   5 each) and fail loudly on a mismatch. Changing a hook's field set means
...
| `code_tap/addummy_in_stepping.F` | `../../MITgcm/pkg/autodiff/addummy_in_stepping.F` | Largest divergence of the set. Upstream is TAF-only and reads adjoint state from `adcommon.h` commons; this version adds `TAP_DUMMY_IN_STEPPING_B` (dumps its adjoint *arguments*; ADEXCH-folds via `stubs_tap_adj.F`) plus a no-op `_D`, and keeps a **condensed** copy of upstream's TAF body in `#else` (MNC/OpenAD and packages this setup never enables dropped) |
...
| `code_tap/flow_tap_local` | *(supplement to `../../MITgcm/tools/TAP_support/flow_tap`, not a shadow)* | Three stanzas declaring the `tap_*` hooks' field arguments active. Compare with the `dummy_in_stepping` stanza in the stock file (all-passive) to see the one change of meaning |
...
| `tap_dummy_in_stepping.F`, `tap_inadmode.F` | the hooks' no-op forward bodies — must never appear in an `*_ad_diff.list` |
| `addummy_in_stepping.F` | `TAP_DUMMY_IN_STEPPING_B`, the adjoint dump body (plus the TAF `ADDUMMY_IN_STEPPING` in its `#else` branch, so the file is upstream-ready) |
