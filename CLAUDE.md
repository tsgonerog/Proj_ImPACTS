...
./build_tapAdj.sh                # -> build_tapAdj_nocheckpoint/mitgcmuv_tap_adj (a symlink: see below)
```

Both setups name scripts action-first (`build_*`, `submit_*`), and the `rawTapenade` control builds are retired in both — raw Tapenade output *is* the working configuration. **In DINO, since 2026-09-02, `build_tapAdj.sh` and `submit_tapAdj.sh` are symlinks** to `build_tapAdj_nocheckpoint.sh` / `submit_tapAdj_nocheckpoint.sh`, the default adjoint; every real DINO adjoint script carries a token saying what it builds:
...
| `build_tapAdj_adjViscBoost.sh` / `submit_tapAdj_adjViscBoost.sh` | `build_tapAdj_adjViscBoost/` | `ckpAll` **plus** the ASTE `inAd*` sources, compiled from `code_tap/variants/adjointViscosity/` as a second `-mods` directory. Deliberately without the list: under the boost, split mode changes the adjoint at order one (run 31056 vs 31025, 2026-09-02 — see the `adjViscBoost` bullet under "Run") |
...
SOMA's `build_tapAdj.sh` / `submit_tapAdj.sh` are real files and still checkpoint every call — no SOMA profile exists yet — so the bare name means different things in the two setups. Optfiles come from `tools/machine_env.sh`, so nothing needs exporting. Every build script does the same steps in the same order, and none of them copies anything into `code/` or `code_tap/` (since 2026-09-02 a build leaves the working tree clean): `make CLEAN`, run **stock** `genmake2` with `-tap -adof=../code_tap/adjoint_tap_local -mods=../code_tap` (the `_nocheckpoint` build adds `-tap_extra "-nocheckpoint \"<list>\""`, the profiling build `-tap_extra "-profile"`; the `adjViscBoost` build passes `-mods="../code_tap/variants/adjointViscosity ../code_tap"` and afterwards asserts the compiled `autodiff_*.f` came from the variant), `make depend`, `make -j 8 tap_adj`, assert every generated hook call's argument count (`check_gen_call`), and — last, so it can only describe a verified executable — write `build_info.txt` into the build directory (script, `TAP_EXTRA`, the list, commit, and a `run_token` such as `tapAdj_ckpAll_adjViscBoost`). The DINO submit scripts refuse an executable that has no record or whose checksum does not match the record's `exe_md5` (since 2026-09-03; the earlier mtime test fired spuriously on the NFS home and survives only as the fallback for records without that line), copy the record into the run directory, and take the run directory's name from `run_token` (see "Where the output lands").
...
- The `AUTODIFF_INADMODE_SET_B`/`UNSET_B` wrappers (and `_D` no-ops) live in `code_tap/dummy_tap.F` with the other hook adjoints, so the plain builds do not shadow `autodiff_inadmode_{set,unset}_ad.F` either.
- **A variant is a second `-mods` directory.** `code_tap/variants/adjointViscosity/` holds the four ASTE-derived shadows under their real MITgcm names (`AUTODIFF_PARAMS.h`, `autodiff_readparms.F`, `autodiff_inadmode_set_ad.F`, `autodiff_inadmode_unset_ad.F`) plus a README; `build_tapAdj_adjViscBoost.sh` passes `-mods="../code_tap/variants/adjointViscosity ../code_tap"`. `genmake2` gives a file in an earlier `-mods` directory preference over a same-named file anywhere later (`genmake2:79-83`), and it enumerates only `*.F *.h *.c *.flow *.F90` directly inside each directory (`genmake2:2952`), so the plain builds never see `variants/`. The build script then asserts that the compiled `autodiff_*.f` really came from the variant. This mirrors `input_tap/variants/adjointViscosity/`, the namelist half of the same configuration, and is the pattern `build_tapAdj_tapProfile.sh` already used for `tools/tapenade_profiling/mods_profile/`. **Both halves were renamed from `adjViscBoost/` on 2026-09-04**, that having been an internal coinage nobody outside this project reads; the build script, the build directory and the `run_token` deliberately keep the older tag, since scratch run directories and every `build_info.txt` already written record it. The rename left `build_tapAdj_adjViscBoost/` with dangling `-mods` symlinks; it was rebuilt, and run 31090 against 31075 (boost, 30 d from rest) is bitwise identical in every `ADJ*`/`adxx_*` file, `fc` and `%MON` line. Inside the variant directory the files carry no suffix because the compiler, not a script, resolves them; the directory name is the tag.
...
`build_tapAdj_adjViscBoost/AUTODIFF_PARAMS.h -> ../code_tap/variants/adjointViscosity/AUTODIFF_PARAMS.h`)
...
**Both setups, upstream shape since 2026-09-02: the upstream hooks keep their names and gain a Tapenade interface.** The pattern covers **four** hooks in DINO — the `ADJ*` dump (`DUMMY_IN_STEPPING`), the `ADJetan` dump (`DUMMY_FOR_ETAN`, a separate hook in `INTEGR_CONTINUITY` because the free-surface adjoint is half a time step out of phase with the rest) and the two adjoint-mode switches (`AUTODIFF_INADMODE_SET`/`UNSET`, whose TAF-named adjoints apply/revert the `inAd*` adjViscBoost parameters at the start/end of every backward step; they were dead code under Tapenade before 2026-08-31, so adjViscBoost never actually boosted) — and the two dump hooks in SOMA, which has no adjViscBoost machinery to switch. The pieces in `code_tap/`, wired identically by every adjoint build script with **stock** `genmake2`:

- `forward_step.F` and `integr_continuity.F` — `-mods` shadows whose only change is the call sites: under `#ifdef ALLOW_TAPENADE` they call `DUMMY_IN_STEPPING(theta, salt, uVel, vVel, wVel, fu, fv, Qnet, Qsw, EmPmR, diffKr, myTime, myIter, myThid)` in the `ALLOW_AUTODIFF_MONITOR` block, `DUMMY_FOR_ETAN(etaN, …)`, and (DINO) `AUTODIFF_INADMODE_UNSET(uVel, …)` at step start / `AUTODIFF_INADMODE_SET(uVel, …)` at step end, with the stock three-argument calls in `#else`. `uVel` in the mode switches is only the activity vehicle that forces `_B` generation.
- `dummy_in_stepping.F`, `dummy_for_etan.F` and (DINO) `autodiff_inadmode_set.F`, `autodiff_inadmode_unset.F` — shadows of the upstream forward no-op hooks whose `SUBROUTINE` statement and declarations gain the wider argument list under `#ifdef ALLOW_TAPENADE` (the idiom `pkg/autodiff` itself uses under `AUTODIFF_TAMC_COMPATIBILITY`); the `#else` branch is the upstream file, so TAF and OpenAD see no change. They stay no-ops and must never appear in an `*_ad_diff.list`.
- `flow_tap_local` — Tapenade external library re-declaring those hooks, under their upstream names, with the field arguments read-then-written (11 fields for the stepping dump, `etaN`, `uVel`). **It must come after the stock `flow_tap` on the Tapenade command line, because Tapenade keeps the last declaration of an external** (tested 2026-09-02: passed first, through `-tap_extra`, the local stanza lost to the stock all-passive one with `TC32 Conflicting numbers of arguments` and no `_B` call was generated). It therefore rides in through `adjoint_tap_local`, a setup-local `-adof` file that sources the stock `tools/adjoint_options/adjoint_tap` (with `ROOTDIR` set by `genmake2`) and appends `-ext ../code_tap/flow_tap_local` to `TAPENADE_FLAGS`; `-tap_extra` now carries only `-nocheckpoint` or `-profile`. Because of it, Tapenade emits `CALL DUMMY_IN_STEPPING_B(theta, thetab, …)` in `forward_step_b.f` and `CALL DUMMY_FOR_ETAN_B(etaN, etaNb, …)` in `integr_continuity_b.f`, each at the reverse-sweep mirror of its forward call.
- `dummy_tap.F` — shadow of `pkg/tapenade/dummy_tap.F`, where upstream ships `DUMMY_IN_STEPPING_B/_D` and `DUMMY_FOR_ETAN_B/_D` as no-op stubs: here they are the hand-written bodies. `DUMMY_IN_STEPPING_B` ADEXCH-folds its adjoint arguments (the `stubs_tap_adj.F` implementations) and dumps them as `ADJ*`; `DUMMY_FOR_ETAN_B` dumps `etaNb` with the separate `dumpAdRecEt` counter (no fold, like upstream); DINO adds the thin `AUTODIFF_INADMODE_SET_B`/`UNSET_B` wrappers (and `_D` no-ops), which only call the TAF-named bodies `ADAUTODIFF_INADMODE_SET`/`UNSET` — the vendored ones in a plain build, the `code_tap/variants/adjointViscosity/` ones in the boost build. The adjoint state arrives as arguments, so there is no `adcommon.h` mirror to keep in sync (the old one is archived in `00_archive/code_tap/`). The TAF files `addummy_in_stepping.F` and `addummy_for_etan.F` are not shadowed at all any more; the vendored ones compile as dead code, exactly as in an upstream Tapenade verification build.

Every shadow keeps the additive convention (upstream content untouched; changes only as guarded call-site switches, guarded interface variants or appended blocks), and the option headers are the upstream c69m headers with only `#define`/`#undef` toggles changed — so `vimdiff` against the counterpart shows exactly the setup's additions, and the setup's diff against the vendored tree is the shape of an upstream patch: two call sites, four hook interfaces, one stanza block in `flow_tap`, and bodies for the stubs in `pkg/tapenade/dummy_tap.F`. From 2026-08-31 to 2026-09-02 the hooks had Tapenade-only names (`TAP_DUMMY_IN_STEPPING`, `TAP_INADMODE_SET`, …) in `tap_*.F` files, with the `_B` bodies appended to shadows of the TAF files; the rename was validated bitwise at 30 d, 5 d (SOMA) and 5 yr (DINO run 31077 vs 31055: `fc`, 32 `adxx_*`, 4 393 `ADJ*`, 18 801 `%MON` lines identical; see the DINO `TODO.md`).

Each `_B` signature is fixed (dump hook: 25 arguments = 11 value/adjoint pairs + 3 passives; etaN dump and mode-switch hooks: 5 each) and **must match what Tapenade generates** — F77 would silently misalign a mismatch, so every build script counts each generated call's arguments after `make` (`check_gen_call`) and fails loudly on a mismatch, and then checks that the compiled `dummy_tap.f` still carries the ten `ADJ*` dump calls — they vanish silently, leaving a bit-correct adjoint that writes no `ADJ*` files, if `dummy_tap.F` loses its `AD_CONFIG.h` include (the only definition of `ALLOW_ADJOINT_RUN`). Changing a hook's field set therefore means touching the call-site shadow, the hook shadow, `flow_tap_local`, the `_B`/`_D` bodies *and* the assertion. The `rawTapenade` control builds are retired in both setups — raw Tapenade output *is* the working configuration now, and no generated file is post-edited.
...
../../../tools/submit.sh submit_tapAdj.sh     # adjoint, committed default (DINO: symlink to submit_tapAdj_nocheckpoint.sh)
...
└── adjoint/{kappa_v_ensemble,checkpointing_study,adjViscBoost,
...
**Three rules keep this from rotting.** (1) A run directory keeps the machine-made name the submit script gave it; the campaign is the directory it sits *in*, never part of the name. (2) Submit scripts write to `runs/forward/` or `runs/adjoint/` directly, so a new run lands unfiled at that level and is moved into a campaign when it joins one — filing a run means updating the notebook that reads it. (3) Output from a notebook that reads **one** run stays inside that run (`<run>/figures/`, `<run>/animations/`); output from a notebook that reads **several** goes to `analysis/<campaign>/`. SOMA has no campaign level yet: three runs do not need one.
...
**Run-directory names.** DINO adjoint directories are `DINO_1deg_<run_token>_<duration>[_<tag>]_run<jobid>` where `<run_token>` is `tapAdj_<ckp>[_<variant>]` (`<ckp>` is `nocheckpoint` or `ckpAll`; `<variant>` `adjViscBoost` or `tapProfile`), carried over from the 2026-09-02 renaming that took the token from the build's `build_info.txt`. Forward and SOMA directories are `<setup>_<mode>_<duration>[_<tag>]_run<jobid>`, with the same configuration tokens the notebooks use (`visc2x`, `viscD2x_Zref`, `viscRef`, `viscGrid<v>`). **The job ID is the durable key** — settings tokens were derived by reading each run's own staged `data` namelist, so if a name and a namelist ever disagree, the namelist wins.
...
fails only on `build_info.txt` is one of those.)
