...
**Both setups, upstream shape since 2026-09-02: the upstream hooks keep their names and gain a Tapenade interface.** The pattern covers **four** hooks in DINO — the `ADJ*` dump (`DUMMY_IN_STEPPING`), the `ADJetan` dump (`DUMMY_FOR_ETAN`, a separate hook in `INTEGR_CONTINUITY` because the free-surface adjoint is half a time step out of phase with the rest) and the two adjoint-mode switches (`AUTODIFF_INADMODE_SET`/`UNSET`, whose TAF-named adjoints apply/revert the `inAd*` adjVisc parameters at the start/end of every backward step; they were dead code under Tapenade before 2026-08-31, so adjVisc never actually boosted) — and the two dump hooks in SOMA, which has no adjVisc machinery to switch. The pieces in `code_tap/`, wired identically by every adjoint build script with **stock** `genmake2`:
