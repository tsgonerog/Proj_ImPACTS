...
[[ -d "notes/$DIR" ]] || { echo "no such direction: notes/$DIR"; exit 1; }

dirty=$(git status --porcelain --untracked-files=no -- "notes/$DIR")
if [[ -n "$dirty" ]]; then
    echo "REFUSING: notes/$DIR has uncommitted changes. This test edits and then"
...
restore() {
    git reset -q -- "notes/$DIR" 2>/dev/null          # in case a test staged something
    git checkout -q -- "notes/$DIR" 2>/dev/null
    git clean -qfd "notes/$DIR" 2>/dev/null
}
...
git archive "HEAD:notes/$DIR" | tar -x -C "$SEED"
git -C "$SEED" add -A
...
victim=$(git ls-files "notes/$DIR" | grep '/sections/.*\.tex$' | head -1)
victim=${victim#notes/$DIR/}
[[ -n "$victim" ]] || { echo "no sections/*.tex in notes/$DIR to test with"; exit 1; }

echo "── argument handling ────────────────────────────────────────────────"
for form in "$DIR" "$DIR/" "notes/$DIR/" "./notes/$DIR"; do
...
st=$(git status --porcelain -- "notes/$DIR")
grep -q "^ M notes/$DIR/$victim" <<<"$st" && ok "edit arrives as ' M'" || bad "edit not reported as modified"
grep -q "^ D notes/$DIR/$doomed" <<<"$st" && ok "deletion arrives as ' D'" || bad "deletion not reported — THE bug this tool exists to prevent"
[[ -e "notes/$DIR/$doomed" ]] && bad "deleted file still on disk" || ok "deleted file gone from disk"

# Untracked build products must survive a pull.
built=$(find "notes/$DIR" -maxdepth 2 -name main.pdf | head -1)
if [[ -n "$built" ]]; then ok "main.pdf survived the pull"; else
    printf "  ${dim}skip  no built PDF present to check${off}\n"; fi
[[ -z "$(git status --porcelain --untracked-files=all -- "notes/$DIR" | grep '^??')" ]] \
    && ok "pull added no untracked files" || bad "pull left untracked files behind"

restore
chk "abort restores the tree" "$(git status --porcelain -- "notes/$DIR")" ""
...
mapfile -t want < <(git ls-files "notes/$DIR" | sed "s|^notes/$DIR/||" | sort)
...
chk "the edit is still uncommitted here" "$(git status --porcelain -- "notes/$DIR/$victim")" " M notes/$DIR/$victim"
...
"$SYNC" pull "$DIR" >/dev/null 2>&1
grep -q 'local only, never pushed anywhere' "notes/$DIR/$victim" \
...
if git rev-parse --verify -q "refs/overleaf-prepull/$DIR" >/dev/null; then
    ...
    git checkout -q "refs/overleaf-prepull/$DIR" -- "notes/$DIR"
    grep -q 'local only, never pushed anywhere' "notes/$DIR/$victim" \
...
echo
restore
chk "tree restored at end" "$(git status --porcelain -- "notes/$DIR")" ""
