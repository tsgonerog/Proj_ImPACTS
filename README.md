...
│
├── build_frd.sh                    ─┐
├── build_tapAdj_nocheckpoint.sh     │  build drivers, one per build directory;
├── build_tapAdj_ckpAll.sh           │  build_tapAdj.sh is a symlink to the
├── build_tapAdj_adjViscBoost.sh     │  current default (_nocheckpoint since
├── build_tapAdj_tapProfile.sh      ─┘  2026-09-02)
│
├── submit_frd.sh                   ─┐
├── submit_tapAdj_nocheckpoint.sh    │  SLURM job scripts, paired 1:1 with the
├── submit_tapAdj_ckpAll.sh          │  build directories; submit_tapAdj.sh is
├── submit_tapAdj_adjViscBoost.sh    │  the matching symlink
├── submit_tapAdj_tapProfile.sh     ─┘
│
...
`ls` groups them rather than interleaving. In DINO the unmarked adjoint names
are **symlinks to the current default pair** (`_nocheckpoint` since
2026-09-02); every real adjoint script carries a token saying what it builds.
Every build script owns exactly one build directory, and every submit script
points at exactly one build directory. Pairing the wrong two silently runs a
...
| `build_tapAdj_adjViscBoost.sh` | `build_tapAdj_adjViscBoost/` | `mitgcmuv_tap_adj` (adjoint-mode viscosity boost, every call checkpointed — the list is not equivalent under the boost) | `submit_tapAdj_adjViscBoost.sh` |
| `build_tapAdj_tapProfile.sh` | `build_tapAdj_tapProfile/` | `mitgcmuv_tap_adj` (diagnostic: ckpAll + Tapenade `-profile`) | `submit_tapAdj_tapProfile.sh` |
